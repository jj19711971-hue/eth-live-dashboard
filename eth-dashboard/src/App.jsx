...
export default function App() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      setScore(s)
      // FIX #5: คำนวณ breakdown จาก indicators จริง
  ...
  // FIX #5: news_score จาก API จริง (ไม่ hardcode)
    ...
    <div style={{ background: '#f2ede4', minHeight: '100vh', maxWidth: 520, margin: '0 auto', paddingBottom: 36, fontFamily: "-apple-system,'Helvetica Neue','Segoe UI',sans-serif" }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
              ...
              {ind?.ethThb && (
                <div style={{ fontSize: 14, color: '#8a7f6e', fontWeight: 600 }}>
      ...
      <Card>
        ...
        <IndRow dotColor="#52b788" label="ADX Strength"
          ...
          valueColor={
            // FIX #2: สีตาม DI direction ไม่ใช่แค่ค่า ADX
            ind?.adx > 25
              ? (ind?.plusDI > ind?.minusDI ? '#2d6a4f' : '#c0392b')
              : '#c07a30'
          }
          bar={{ value: ind?.adx ?? 0, max: 60,
            color: ind?.adx > 25
              ? (ind?.plusDI > ind?.minusDI ? '#52b788' : '#e63946')
              : '#f4a261'
          }} />
      ...
      <Card>
        ...
        {/* FIX #3: Funding Rate จาก API จริง */}
      ...
      {/* SUMMARY — FIX #5: สีและ ✓/✗ ตามค่าจริง */}
      <Card>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          {[
            ...
            <div key={i} style={{
              ...
              // FIX #5: background สีตามสถานะจริง
        ...
        {/* FIX #6: แสดง S/R จาก Swing จริง พร้อม disclaimer */}
